...
saveRDS(subClusters, file="~/Desktop/GIT/R_packages_project/MultiConnector/inst/Data/MCL/MCLTwoD_SubClustering.rds")
...
library(dplyr)
library(MultiConnector)
subClusters <- readRDS("~/Desktop/GIT/R_packages_project/MultiConnector/inst/Data/MCL/MCLTwoD_SubClustering.rds")
subClusterData <- selectCluster(subClusters, G=3, "MinfDB")
plot(subClusterData,feature = "TTP")
...
merge(newCurves, Annotations, by="subjID") %>% select(cluster,TTP,subjID) %>% distinct() %>%
  group_by(cluster, TTP) %>% summarise(n=n()) %>% ungroup() %>% tidyr::pivot_wider(names_from = TTP, values_from = n, values_fill = 0)


Dataset = merge(newCurves, Annotations, by="subjID") %>% select(cluster,TimeTTPevent,TTP,subjID) %>% distinct()
...
TimeSeries %>%  ggplot() + 
  ...
  facet_wrap( ~ measureID, scale = "free")+ labs(x = "Time (days)", y = "MRD quantification" ) +
  theme_bw() +
  scale_color_manual(values = c("Subject 201" = "blue")) +
  theme(
    axis.title.x = element_text(
...
merge(newCurves, Annotations, by="subjID") %>% select(cluster,Arm,subjID) %>% distinct() %>%
  group_by(cluster, Arm) %>% summarise(n=n()) %>% ungroup() %>% tidyr::pivot_wider(names_from = Arm, values_from = n, values_fill = 0)
