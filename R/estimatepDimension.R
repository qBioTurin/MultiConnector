...
setMethod("estimatepDimension",
  ...
  function(data,
    ...
    finalres$p_suggested <- apply(finalres$summary, 1, first_relmax_col, colnames_x = colnames(finalres$summary), tol = 0.01)
...
setMethod("estimatepDimensionPerObs",
  signature = c("CONNECTORData"),
  function(data, p, cores) {
    ...
    if (length(unique(data@dimensions$curvesID)) > 9) {
      ...
      splits <- length(unique(data@dimensions$curvesID))
    }
    ...
    Knots.list <- lapply(p, function(p) {
      ...
      df <-
        tibble(
          Knots = c(attr(Spline, "Boundary.knots"), attr(Spline, "knots")),
    ...
    GrowthCurve <-
      ...
      scale_x_continuous(breaks = as.integer(seq(
        min(grid), max(grid),
        length.out = max(2, length(grid) / 5)
      ))) +
      geom_line() +
      geom_point() +
      labs(x = "", y = "") +
    ...
    ValidationPlot <- ggplot() +
      ...
      geom_point(
        data = meanCrossvalidData, aes(x = p, y = mean), size =
          2
      ) +
    ...
    time_unit <- attr(time_diff, "units")
    print(paste(
      "Total time for ", unique(data@curves$measureID[data@curves$curvesID %in% curve]),
