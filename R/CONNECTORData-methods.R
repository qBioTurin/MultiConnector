...
setMethod(
  ...
  function(TimeSeriesFile, AnnotationFile) {
    ...
    if (grepl("\\.xlsx$", TimeSeriesFile, ignore.case = TRUE) ||
      grepl("\\.xls$", TimeSeriesFile, ignore.case = TRUE)) {
      curves <- readxl::read_excel(TimeSeriesFile)
    } else if (grepl("\\.csv$", TimeSeriesFile, ignore.case = TRUE)) {
      curves <- readr::read_csv(TimeSeriesFile, col_names = TRUE, show_col_types = FALSE)
    } else if (grepl("\\.txt$", TimeSeriesFile, ignore.case = TRUE) ||
      grepl("\\.tsv$", TimeSeriesFile, ignore.case = TRUE)) {
      curves <- readr::read_delim(TimeSeriesFile, delim = NULL, col_names = TRUE, show_col_types = FALSE)
    } else {
      warning("The TimeSeriesFile must be an excel, csv, txt or tsv file")
      return()
    }

    if (grepl("\\.xlsx$", AnnotationFile, ignore.case = TRUE) ||
      grepl("\\.xls$", AnnotationFile, ignore.case = TRUE)) {
      annotations <- readxl::read_excel(AnnotationFile)
    } else if (grepl("\\.csv$", AnnotationFile, ignore.case = TRUE)) {
      annotations <- readr::read_csv(AnnotationFile, col_names = TRUE, show_col_types = FALSE)
    } else if (grepl("\\.txt$", AnnotationFile, ignore.case = TRUE) ||
      grepl("\\.tsv$", AnnotationFile, ignore.case = TRUE)) {
      annotations <- readr::read_delim(AnnotationFile, delim = NULL, col_names = TRUE, show_col_types = FALSE)
    } else {
...
setMethod(
  ...
  function(TimeSeriesFile, AnnotationFile) {
    ...
    subjIDMeasure <- curves %>%
      select(subjID, measureID) %>%
      distinct() %>%
      count(subjID) %>%
      filter(n != NumMeasures) %>%
      pull(subjID)
    ...
    M <- sort(as.character(unique(curves$measureID)))
    curves$measureID <- factor(curves$measureID, levels = M)
    curves <- curves %>% arrange(subjID, measureID, time)
