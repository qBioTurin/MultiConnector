...
This section demonstrates the new advanced analysis capabilities available in MultiConnector for detailed subject-level analysis and comprehensive reporting.

## Subject-Level Analysis with SubjectInfo()

The `SubjectInfo()` function allows detailed analysis of specific subjects within their clustering context, providing highlighted visualizations and comprehensive information.

```{r subject-analysis, fig.width=12, fig.height=8}
# Select some subjects for detailed analysis
selected_subjects <- unique(ovarian_data@annotations$subjID)[1:3]

# Single subject analysis
subject_info <- SubjectInfo(final_result, subjIDs = selected_subjects[1])
...
multi_subject_info <- SubjectInfo(final_result, subjIDs = selected_subjects)
multi_subject_info$highlighted_plot

# Subject analysis with feature coloring
if ("Progeny" %in% colnames(ovarian_data@annotations)) {
  feature_analysis <- SubjectInfo(final_result, 
                                 subjIDs = selected_subjects[1],
                                 feature = "Progeny",
                                 feature_type = "discrete")
  feature_analysis$highlighted_plot
}
```

The `SubjectInfo()` function provides:

- **Cluster assignment**: Which cluster each subject belongs to
- **Highlighted visualization**: Subject curves emphasized in cluster context
- **Quality metrics**: Access to silhouette and entropy measures
- **Subject data**: Complete time series data for the specified subjects

## Cluster Composition Analysis with clusterDistribution()

Analyze how different features are distributed across clusters using cross-tabulation:

```{r cluster-distribution}
# Analyze progeny distribution across clusters (if available)
if ("Progeny" %in% colnames(ovarian_data@annotations)) {
  progeny_dist <- clusterDistribution(final_result, "Progeny")
  ...
  cat("\nAnalysis Details:")
  cat("\nTotal subjects:", attr(progeny_dist, "total_subjects"))
  cat("\nMissing values:", attr(progeny_dist, "missing_values"))
}

# If other features are available, analyze them too
available_features <- getAnnotations(final_result)
...
if (length(available_features) > 1) {
  ...
  if (demo_feature != "Progeny") {  # Avoid duplicate analysis
    tryCatch({
      feature_dist <- clusterDistribution(final_result, demo_feature)
...
The `clusterDistribution()` function helps identify:

- **Feature-cluster associations**: Which features are enriched in specific clusters
- **Cluster balance**: Whether clusters are balanced across important covariates
- **Quality control**: Ensuring clustering isn't driven by confounding variables

## Comprehensive Report Generation

Generate a complete analysis report containing all plots, tables, and analysis choices:

```{r comprehensive-report, fig.width=12, fig.height=10}
# Generate comprehensive report
comprehensive_report <- generateReport(
  data = ovarian_data,                      # Include dimension analysis
  clustered_data = final_result,            # Include clustering results
  report_title = "Ovarian Cancer Growth Patterns Analysis",
  features = getAnnotations(final_result),  # Analyze all available features
...
# Access specific report elements
cat("\nReport contains:")
cat("\n- Plots:", length(comprehensive_report$plots))
cat("\n- Tables:", length(comprehensive_report$tables))
cat("\n- Analysis sections:", length(comprehensive_report$analysis_choices))

# Show some key plots from the report
if (!is.null(comprehensive_report$plots$timeseries_plot)) {
  cat("\n\n### Time Series Overview from Report:")
  print(comprehensive_report$plots$timeseries_plot)
}

if (!is.null(comprehensive_report$plots$cluster_plot_basic)) {
  cat("\n\n### Basic Cluster Plot from Report:")
  print(comprehensive_report$plots$cluster_plot_basic)
}

# Display cluster statistics table
if (!is.null(comprehensive_report$tables$cluster_assignments)) {
  cat("\n\n### Cluster Assignment Summary:")
  print(comprehensive_report$tables$cluster_assignments)
}
```

The `generateReport()` function creates a comprehensive analysis containing:

- **Data overview**: Summary statistics and basic visualizations
- **Dimension analysis**: Optimal spline dimension selection plots
- **Clustering results**: All cluster-related plots and quality metrics
- **Feature analysis**: Feature-specific visualizations and distributions
- **Quality assessment**: Complete validation metrics and tables
- **Analysis metadata**: Parameters used and methodological choices

This report serves as a complete record of your analysis and can be used for:

- **Documentation**: Complete analysis provenance
- **Sharing**: Comprehensive results for collaborators
- **Reproducibility**: All analysis choices and parameters recorded
- **Quality control**: Systematic review of all analysis components
